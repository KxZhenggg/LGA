...
class UPET_Latent_Opt_Calculator(UPETCalculator):
    """
    UPET Latent Vector Optimization Calculator.
    基于 UPETCalculator，实现对 metatomic System 的全微分支持（Positions + Strain）。
    """
    def __init__(
        self,
        ...
        opt_coeff: float = 1.0,
        cutoff: float = 10.0, # 默认截断半径，需根据模型设定调整
        **kwargs
    ):
        ...
        try:
            ...
            self.device = 'cpu'
            self.dtype = torch.float64 # default for metatomic usually
    ...
    def forward_latent_with_grad(self, atoms):
        """
        核心函数：
        1. 定义 Pos 和 Strain 叶子节点
        2. 显式计算变形后的 Pos 和 Cell
        3. 重建可微的邻居列表 (Edge Vectors)
        4. 包装成 Metatomic System 并运行
        """
        ...
        cell_orig = torch.tensor(cell_array, dtype=self.dtype, device=self.device) # 常量
...
def main():
    ...
    calc = UPET_Latent_Opt_Calculator(
        model="pet-oam-xl", 
        checkpoint_path="pet-oam-xl-v1.0.0.ckpt", 
        device="cuda",
        opt_coeff=1.0
    )
