...
class MatterSim_Latent_Opt_Calculator(Calculator):
    """
    MatterSim Latent Vector Optimization Calculator
    优化几何结构 (包括晶胞) 以匹配目标 MatterSim 描述符。
    """
    ...
    def forward_latent_with_grad(self, atoms):
        """
        执行前向传播，通过同时变形 atom_pos 和 cell 来实现应力 (Stress) 计算。
        """
        ...
        if "atom_pos" in input_data:
            ...
            raise RuntimeError("输入字典中缺少 'atom_pos'。")
    ...
    def calculate(self, atoms=None, properties=None, system_changes=all_changes):
        ...
        forces_grad = grads[0]
        stress_grad = grads[1] # 这是 Virial (dE/d_strain)
